...
st.title('Data visualiseringer for Arbejdstilsynets data')
...
def plot_top_and_bottom_groups(dataframe, hsg_value, kilde):
    ...
    fig.update_layout(title=f'Øverste og nederste 5 grupper ved gennemsnitlig score for {kilde}. Høj score godt = {hsg_value}', xaxis_title='Gennemsnitlig score', yaxis_title='Group')
    st.plotly_chart(fig)
...
Grafen viser, at det optimale antal clusters er 4, som afsløres af de fire røde centroider, der er markeret i den 3D-graf. Dette er interessant, da man kunne have argumenteret for, at der skulle være 10 clusters (2 x køn og 5 x aldersgrupper = 10). En visuel analyse af k-means clusteringen viser noget meget fascinerende. En af clusterne, den markeret med den mørkeblå farve, indeholder data fra begge køn. Alle de andre clusters indeholder kun data fra ét køn. Derudover omfatter de mørkeblå og sky-blå farvede clusters data fra alle aldersgrupper. En dybere analyse af dette kunne undersøge, om den gule cluster måske repræsenterer en branche eller en anden variabel, der ikke er medtaget i denne clustering.
...
grouped_data_topic = branche_data_filtered.groupby(['Topic Label', 'Hoej Score Godt'])['Score'].mean().reset_index()
